          ...
          <table class="orders-table">
            ...
            <tbody>
              <tr v-for="order in restockingOrders" :key="order.id">
                ...
                <td class="col-status">
                  <span class="badge restocking">
                    Restocking
                  </span>
                </td>
                <td class="col-date">{{ formatDate(order.order_date) }}</td>
                <td class="col-date">{{ getLeadTime(order.warehouse) }}</td>
...
export default {
  ...
  setup() {
    ...
    const restockingOrders = computed(() => {
      return orders.value.filter(order => order.status === 'Restocking')
    })
    ...
    const getLeadTime = (warehouse) => {
      const leadTimes = {
        'San Francisco': () => {
          const min = 3
          const max = 5
          return `${Math.floor(Math.random() * (max - min + 1)) + min}-${max} days`
        },
        'London': () => '5-7 days',
        'Tokyo': () => '7-10 days'
      }

      return leadTimes[warehouse] ? leadTimes[warehouse]() : '5-7 days'
    }
